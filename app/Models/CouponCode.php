    	...
    	/*
			select * from orders where user_id = xx and coupon_code_id = xx
  				and (
    				...
    				or 
    				( paid_at is not null and refund_status = 'pending' )
 				)
    	*/
    				...
    				->where(function($query){
    					$query->where(function($query){
    						...
    						$query->whereNotNull('paid_at')
    							->where('refund_status',Order::REFUND_STATUS_PENDING);//已付款且未退款订单
    				});
    			})->exists();
    	if($used){
    		throw new CouponCodeUnavailableException('你已经使用过这种优惠券了');
    	}
    }
